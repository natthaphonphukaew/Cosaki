...
export default function PaymentMethods() {
  ...
  const addCard = () => {
    ...
    const card = { id: Date.now(), last4: digits.slice(-4), name: form.name || 'Cardholder', exp: form.exp || '12/29' };
      ...
      <div className="px-4 pt-4 space-y-3">
        {cards.length === 0 && (
          <div className="flex flex-col items-center rounded-2xl bg-white py-10 text-center shadow-sm">
            ...
            <p className="text-sm font-medium text-gray-600">No cards saved</p>
          </div>
        )}
        ...
        <p className="px-1 text-center text-xs text-gray-400">For demo only — no real card is charged.</p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30" onClick={() => setAdding(false)}>
          <div className="w-full max-w-[390px] space-y-3 rounded-t-3xl bg-white p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-900">Add Card</h3>
              <button onClick={() => setAdding(false)} className="text-gray-400"><X size={20} /></button>
            </div>
            <Input label="Card number" value={form.number} onChange={(e) => setForm({ ...form, number: e.target.value })} placeholder="4242 4242 4242 4242" />
            <Input label="Cardholder name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            <Input label="Expiry" value={form.exp} onChange={(e) => setForm({ ...form, exp: e.target.value })} placeholder="MM/YY" />
            <Button className="mt-2 w-full" onClick={addCard}>Add Card</Button>
          </div>
        </div>
      )}
    </div>
