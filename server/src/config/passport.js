...
// ── Google OAuth Strategy (optional — only registered when credentials exist) ──
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          ...
          const { rows: newUser } = await db.query(
            ...
            [email, profile.id, profile.displayName, profile.photos?.[0]?.value || null]
          );
...
// ── Facebook OAuth Strategy (optional — only registered when credentials exist) ─
        ...
        try {
          ...
          if (rows.length) {
            if (!rows[0].facebook_id) {
              await db.query('UPDATE users SET facebook_id = $1 WHERE id = $2', [profile.id, rows[0].id]);
            }
            return done(null, rows[0]);
          }
          const { rows: newUser } = await db.query(
